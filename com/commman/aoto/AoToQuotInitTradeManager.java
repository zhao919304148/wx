 ...
 *            
 */
public class AoToQuotInitTradeManager {
	...
	public TradeManager initTradeManagerAdviance() {
		...
		// TODO Auto-generated constructor stub
		 try {		
			String trfile = SystemParameter.getInstance().getTradeFile() ;  //交易配置
		    ...
		    //响应码节点（含路径）
			xbi.setResponseCodeAttr("/PACKET/HEAD/RESPONSE_CODE" );
		    //错误消息节点（含路径）
			xbi.setErrorMessageAttr("/PACKET/HEAD/RESPONSE_MESSAGE");
		    //主机交易码节点(含路径)
			xbi.setHostCodeAttr("/PACKET/HEAD/TRANS_TYPE");
		    
			//设置成功标志的值，如果不在此范围解析返回xml中的head时均定为错误 ，
			// []中的值表示成功标志
			xbi.getResponseSuccFlag().append("[0]");
			xbi.getResponseSuccFlag().append("[1]");
	...
	static public void main(String args[]) {
		AoToQuotInitTradeManager doRevDataService = new AoToQuotInitTradeManager();
